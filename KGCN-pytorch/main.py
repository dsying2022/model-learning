...
for epoch in range(args.n_epochs):
    ...
    with torch.no_grad():
        ...
        for user_ids, item_ids, labels in test_loader:
            ...
            scores = outputs.cpu().numpy() #predict socres,numpy(1 x batchsize)
            scores[scores >= 0.5] = 1
            scores[scores < 0.5] = 0
            #f1_score_list.append(f1_score(y_true=labels.numpy(), y_pred=scores))
            test_f1 += f1_score(y_true=labels.cpu().numpy(), y_pred=scores)

        auc_score_list.append(test_roc / len(test_loader))
        f1_score_list.append(test_f1 / len(test_loader))
        for user_ids, item_ids, labels in val_loader:
            ...
            scores[scores >= 0.5] = 1
            scores[scores < 0.5] = 0
            val_f1 += f1_score(y_true=labels.cpu().numpy(), y_pred=scores)
    
            #total_f1 += f1_score(y_true=labels.cpu().detach().numpy(), outputs)
        #print('[Epoch {}]test_loss: '.format(epoch+1), test_loss / len(test_loader))
        auc_score_list.append(val_roc / len(val_loader))
        f1_score_list.append(val_f1 / len(val_loader))
